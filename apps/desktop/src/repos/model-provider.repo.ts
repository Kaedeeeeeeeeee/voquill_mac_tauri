...
export class MistralModelProviderRepo extends BaseModelProviderRepo {
  supportsGenerativeTextModels(): boolean {
    return false;
  }
  ...
  async getTranscriptionModels(options: FetchModelsOptions): Promise<string[]> {
    const fetched = await this.fetchModels(options);
    const voxtral = fetched.filter((m) => m.toLowerCase().includes("voxtral"));
    return filterFetchedModels(voxtral, MISTRAL_TRANSCRIPTION_MODELS);
  }
}
