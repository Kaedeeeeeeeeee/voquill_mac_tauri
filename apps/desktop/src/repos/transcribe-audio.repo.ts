...
import { TranscriptionMode } from "../types/ai.types";
...
export class MistralTranscribeAudioRepo extends BaseTranscribeAudioRepo {
  private apiKey: string;
  private model: MistralTranscriptionModel;

  constructor(apiKey: string, model: string | null) {
    super();
    this.apiKey = apiKey;
    this.model = (model as MistralTranscriptionModel) ?? "voxtral-mini-latest";
  }

  // Mistral handles up to 3-hour recordings; 60s segments keep memory low.
  protected getSegmentDurationSec(): number {
    return 60;
  }
  ...
  protected async transcribeSegment(
    ...
    const { text: transcript } = await mistralTranscribeAudio({
      apiKey: this.apiKey,
      model: this.model,
      blob: wavBuffer,
      ext: "wav",
