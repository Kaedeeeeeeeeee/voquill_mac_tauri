...
const getGenTextRepoInternal = ({
  ...
  if (prefs.mode === "cloud") {
    ...
    const repo: BaseGenerateTextRepo = new CloudGenerateTextRepo(cloudModel);
    return {
      repo,
      apiKeyId: null,
...
export const getTranscribeAudioRepo = (): TranscribeAudioRepoOutput => {
  ...
  if (prefs.mode === "cloud") {
    const repo: BaseTranscribeAudioRepo = new NewServerTranscribeAudioRepo();
    return {
      repo,
      apiKeyId: null,
      warnings: prefs.warnings,
    ...
    } else if (prefs.provider === "mistral") {
      repo = new MistralTranscribeAudioRepo(
        prefs.apiKeyValue,
        prefs.transcriptionModel,
      );
  ...
  throw new Error(
    `Unsupported transcription mode: ${(prefs as { mode: string }).mode}`,
  );
};
...
export const getModelProviderRepo = (
  ...
  switch (provider) {
    ...
    case "mistral":
      return new MistralModelProviderRepo();
    default:
