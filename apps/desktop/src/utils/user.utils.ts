...
export const getEffectiveTranscriptionMode = (
  ...
  // Local mode is currently disabled in the UI; any persisted "local" value
  // (from older sessions where we tried WhisperKit) is coerced to cloud.
  const mode = resolveMode(state, state.settings.aiTranscription.mode, "cloud");
  return mode === "local" ? "cloud" : mode;
};
